...
static void bst_inorder_aux(const bst_node *node, data_ptr *items, int *index) {
    if (node != NULL) {
        ...
        items[*index] = malloc(sizeof(**items));
...
int bst_leaf_count(const bst_linked *source) {
    ...
    // your code here
    bst_linked *temp = (bst_linked *)source;
    return bst_leaf_count_aux(temp->root);

}

// auxillary function
int bst_leaf_count_aux(bst_node *node) {
    if(node == NULL) {
        return 0;
    }
...
BOOLEAN bst_balanced(const bst_linked *source) {
    ...
    int left_height = bst_node_height(source->root->left);
    int right_height = bst_node_height(source->root->right);
