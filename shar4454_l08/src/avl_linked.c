...
static BOOLEAN avl_insert_aux(avl_linked *source, avl_node **node, const data_ptr item) {
    ...
    // your code here
    BOOLEAN insertion_result = FALSE;
    ...
    if (*node == NULL) {
        *node = avl_node_initialize(item);
        ...
        int comparison_result = data_compare(item, (*node)->item);
        if (comparison_result < 0) {
            insertion_result = avl_insert_aux(source, &((*node)->left), item);
        } else if (comparison_result > 0) {
            insertion_result = avl_insert_aux(source, &((*node)->right), item);
        }
...
static BOOLEAN avl_remove_aux(avl_linked *source, avl_node **node, const data_ptr key, data_ptr item) {
    ...
    // your code here
    BOOLEAN removal_result = FALSE;

    if (*node == NULL) {
        int comparison_result = data_compare(key, (*node)->item);

        if (comparison_result < 0) {
            removal_result = avl_remove_aux(source, &((*node)->left), key, item);
        } else if (comparison_result > 0) {
            removal_result = avl_remove_aux(source, &((*node)->right), key, item);
        } else {
            avl_node *temp = *node;
            ...
            if ((*node)->left == NULL) {
                ...
                *node = (*node)->left;
                ...
                (*node)->item = pred->item;
                removal_result = avl_remove_aux(source, &((*node)->left), pred->item, item);
            }
